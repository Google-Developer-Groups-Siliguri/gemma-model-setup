# GDG Siliguri Hackathon: Windows Offline AI Setup
...
Windows users can install the runner easily. It runs as a background service and will automatically detect NVIDIA GPUs if Game Ready drivers are installed. Open PowerShell and use the Windows Package Manager (Winget):

```powershell
winget install Ollama.Ollama
```
*(Alternatively, download `OllamaSetup.exe` directly from ollama.com/download/windows and run the installer).*

## Step 2: Pull the Models
Open Command Prompt or PowerShell and download the models:
...
Turn off your Wi-Fi to test offline inference:

```powershell
ollama run gemma4:e4b
```

When the `>>>` prompt appears, ask it a backend or full-stack question. For instance: *"Write a simple Node.js script using the Genkit framework to interact with a local AI model."*
*(Type `/bye` to exit).*
